        ...
        while n_x < len(XJJ): # CHECKS WHETHER X==XJJ
            ...
            if XSS is None:
                ...
                cache.sort(key=len)
                    ...
                    # print()
            # print('wo', X, XJJ, XSS, g_prime)
            if len(XJJ) == len(XSS):
                fdt.add_fd(X, XJJ)
                # print('\n::',X, XJJ, m_i)
                for gp in cache:
                    bt.append([x in gp for x in U], True)
                break
            else:
                gp = cache.pop()
                bt.append([x in gp for x in U], False)
                n_gp = len(g_prime)
                XJ.add(n_gp)
                for i in stack[1:]:
                    i[1].add(n_gp)
