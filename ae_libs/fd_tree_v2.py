...
class FDTree(object):
    ...
    def add_fd(self, ant, con):
        ...
        while bool(ant):
            x = ant.pop()
            if not current_node[0].get(x, False):# current_node[0][x] is None:
                # current_node[0][x] = [[None]*self.n_attributes, [False]*self.n_attributes]
                current_node[0][x] = [{}, set([])]
            # print(current_node)
    ...
    def read_and_iterate(self, current_node, base):
        ...
        for i in current_node[0]:#, j in filter(lambda k: type(k[1]) == list, enumerate(current_node[0])):
    ...
    def l_close_and_iterate(self, current_node, ant):
        ...
        for i in ant:# current_node[0]:
            # if current_node[1][i] and i not in ant:
            #     yield i
            if current_node[0].get(i, False):# is not None and i in ant:
